...
| 文件类型 | 默认打开方式 | 快捷键 | 说明 |
|---------|------------|--------|------|
| Markdown 文件 | Typora | `Enter` | 用 Typora 打开编辑 |
| 其他文本文件 | nvim | `Enter` | 在终端中打开编辑 |
| 图片文件 | XnView MP | `Enter` | 用 XnView MP 查看 |
| 音视频 | Windows 默认播放器 | `Enter` | 用默认播放器打开 |
...
**操作**:
1. 用 `j`/`k` 选择 Markdown 文件
2. 按 `Enter` - 用 Typora 打开
3. 按 `o` - 显示打开方式菜单，可选择：
   - **Typora** (默认)
   - nvim
...
**说明**:
- 按 `Enter` 会在新的资源管理器窗口中打开该文件夹
- 按 `l` 是在 yazi 内部进入该目录，继续浏览
